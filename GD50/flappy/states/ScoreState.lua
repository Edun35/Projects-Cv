...
function ScoreState:render()
    ...
    if self.score < 3 then
      love.graphics.draw(self.Bronze, VIRTUAL_WIDTH / 2 - (self.Bronze:getWidth() / 2), 120)
    end
    if self.score > 3 then
      love.graphics.draw(self.Silver, VIRTUAL_WIDTH / 2 - (self.Silver:getWidth() / 2), 120)
    end
    if self.score > 5 then
      love.graphics.draw(self.Gold, VIRTUAL_WIDTH / 2 - (self.Gold:getWidth() / 2), 120)
    end
