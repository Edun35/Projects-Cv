...
function PlayState:update(dt)
    ...
    if self.canInput then
        ...
        local boardmouseX = math.floor((mouseX -self.board.x)/32)
        local boardmouseY = math.floor((mouseY - self.board.y)/32)

        if 0 <= boardmouseX and boardmouseX <= 7 and 0 <= boardmouseY and boardmouseY <= 7 then
           self.boardHighlightX = boardmouseX
           self.boardHighlightY = boardmouseY 
        end
