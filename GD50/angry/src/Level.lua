...
function Level:update(dt)
    ...
    self.launchMarker:update(dt, self.col)
    ...
    if self.launchMarker.launched then
        ...
        if self.launchMarker.split then
            local xPos1, yPos1 = self.launchMarker.aliens[1].body:getPosition() or 0,0
            local xVel1, yVel1 = self.launchMarker.aliens[1].body:getLinearVelocity() or 0,0
            local xPos2, yPos2 = self.launchMarker.aliens[2].body:getPosition() or 0,0
            local xVel2, yVel2 = self.launchMarker.aliens[2].body:getLinearVelocity() or 0,0
            
                -- if we fired our alien to the left or it's almost done rolling, respawn
                if xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 1.5) or xPos > 900 then
                    if xPos1 < 0 or (math.abs(xVel1) + math.abs(yVel1) < 1.5) or xPos > 900 then
                        if xPos2 < 0 or (math.abs(xVel2) + math.abs(yVel2) < 1.5) or xPos > 900 then
                        self.launchMarker.aliens[2].body:destroy()
                        table.remove(self.launchMarker.aliens, 2)
                        end
                        self.launchMarker.aliens[1].body:destroy()
                        table.remove(self.launchMarker.aliens, 1)
                    end
            ...
            -- if we fired our alien to the left or it's almost done rolling, respawn
            if xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 1.5) or xPos > 800 then     
                self.launchMarker.alien.body:destroy()
                self.col = false
                self.launchMarker = AlienLaunchMarker(self.world)
                -- re-initialize level if we have no more aliens
